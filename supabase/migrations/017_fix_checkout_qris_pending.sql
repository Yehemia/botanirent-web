-- ============================================================
-- Migration 017: Fix Checkout RPC - Respect payment_status from payload
-- Sebelumnya RPC selalu hardcode 'paid', menyebabkan QRIS selalu terbaca lunas
-- Sekarang menggunakan payment_status dari payload (bisa 'pending' untuk QRIS)
-- ============================================================
...
BEGIN
    ...
    -- Tentukan payment status dari payload (default: 'paid' untuk backward compatibility)
    v_payment_status := COALESCE(NULLIF(payload->>'payment_status', ''), 'paid');
        ...
        ELSIF v_item->>'type' = 'rental' THEN
            ...
            FOR v_i IN 1..v_qty LOOP
                ...
                INSERT INTO public.transaction_items (
                    ...
                    (v_item->>'unit_price')::numeric * ((payload->>'rental_days')::int),
