-- ============================================================
-- Migration 016: Link Bookings to Transaction Items
-- Menyimpan transaction_item_id di tabel bookings saat checkout
-- agar kalender booking dapat menampilkan detail pelanggan/transaksi.
-- ============================================================
    ...
    LOOP
        ...
        ELSIF v_item->>'type' = 'rental' THEN
            ...
            FOR v_i IN 1..v_qty LOOP
                ...
                -- Insert booking (kalender ketersediaan, terhubung ke transaction_item_id)
        ...
        ELSIF v_item->>'type' = 'package' THEN
            ...
            INSERT INTO public.transaction_items (
                ...
                v_item->>'item_name', v_qty, (v_item->>'unit_price')::numeric, (v_item->>'unit_price')::numeric * v_qty,
                (payload->>'rental_start_date')::date, (payload->>'rental_end_date')::date, (payload->>'rental_days')::int, 'active'
            );
            ...
            FOR v_pkg_item IN SELECT * FROM public.package_items WHERE package_id = (v_item->>'package_id')::uuid LOOP
                ...
                FOR v_i IN 1..(v_qty * v_pkg_item.quantity) LOOP
                    ...
                    -- Insert booking (terhubung ke transaction_item_id)
                    INSERT INTO public.bookings (
                        rental_asset_id, transaction_item_id, branch_id, start_date, end_date, status
                    ) VALUES (
                        v_asset_record.id, v_transaction_item_id, (payload->>'branch_id')::uuid, (payload->>'rental_start_date')::date, (payload->>'rental_end_date')::date, 'active'
